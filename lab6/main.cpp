#include <cmath>
#include <cctype>
#include <cstdio>
#include <cstdlib>
#include <string>
#include <vector>

#ifdef __APPLE__
#include <GLUT/glut.h>
#else
#include <GL/glut.h>
#endif

// --------- Scene state ---------
static float g_faceGap = 0.0f;            // separation of cube faces
static float g_orbitAngle = 0.0f;         // light angle around cube (deg)
static float g_orbitSpeed = 30.0f;        // deg/sec
static float g_orbitPlaneYaw = 25.0f;     // rotate orbit plane around Y (deg)
static float g_orbitPlanePitch = 20.0f;   // rotate orbit plane around X (deg)

static float g_camYaw = 35.0f;            // camera orbit
static float g_camPitch = 20.0f;
static float g_camDist = 6.5f;

static bool g_transparent = false;        // toggle face transparency
static bool g_useTextures = true;         // toggle textures
static bool g_lightingEnabled = true;     // toggle lighting

static int g_lastTimeMs = 0;

static GLuint g_tex[2] = {0, 0};

static float faceAlpha() {
    return g_transparent ? 0.45f : 1.0f;
}

static bool readToken(FILE* f, std::string& out) {
    out.clear();
    int c = 0;
    while ((c = fgetc(f)) != EOF) {
        if (std::isspace(c)) {
            continue;
        }
        if (c == '#') {
            while ((c = fgetc(f)) != EOF && c != '\n') {}
            continue;
        }
        out.push_back(static_cast<char>(c));
        break;
    }
    if (out.empty()) {
        return false;
    }
    while ((c = fgetc(f)) != EOF) {
        if (std::isspace(c)) {
            break;
        }
        if (c == '#') {
            while ((c = fgetc(f)) != EOF && c != '\n') {}
            break;
        }
        out.push_back(static_cast<char>(c));
    }
    return true;
}

static bool loadPPM(const char* path, int& w, int& h, std::vector<unsigned char>& data) {
    FILE* f = std::fopen(path, "rb");
    if (!f) {
        return false;
    }

    std::string token;
    if (!readToken(f, token) || token != "P6") {
        std::fclose(f);
        return false;
    }
    if (!readToken(f, token)) { std::fclose(f); return false; }
    w = std::atoi(token.c_str());
    if (!readToken(f, token)) { std::fclose(f); return false; }
    h = std::atoi(token.c_str());
    if (!readToken(f, token)) { std::fclose(f); return false; }
    int maxv = std::atoi(token.c_str());
    if (w <= 0 || h <= 0 || maxv <= 0) {
        std::fclose(f);
        return false;
    }

    data.resize(static_cast<size_t>(w) * static_cast<size_t>(h) * 3u);
    size_t read = std::fread(data.data(), 1, data.size(), f);
    std::fclose(f);
    if (read != data.size()) {
        return false;
    }

    if (maxv != 255) {
        for (size_t i = 0; i < data.size(); ++i) {
            data[i] = static_cast<unsigned char>((data[i] * 255) / maxv);
        }
    }
    return true;
}

static void makeChecker(int w, int h, std::vector<unsigned char>& data) {
    data.resize(static_cast<size_t>(w) * static_cast<size_t>(h) * 3u);
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            int cell = ((x / 8) + (y / 8)) % 2;
            unsigned char c = cell ? 220 : 40;
            size_t idx = (static_cast<size_t>(y) * static_cast<size_t>(w) + static_cast<size_t>(x)) * 3u;
            data[idx + 0] = c;
            data[idx + 1] = static_cast<unsigned char>(c / 2 + 60);
            data[idx + 2] = static_cast<unsigned char>(255 - c / 3);
        }
    }
}

static GLuint createTexture(int w, int h, const std::vector<unsigned char>& data) {
    GLuint id = 0;
    glGenTextures(1, &id);
    glBindTexture(GL_TEXTURE_2D, id);
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, data.data());
    return id;
}

static void initTextures() {
    // Put your P6 PPM files next to the executable or change these paths.
    const char* files[2] = {"texture1.ppm", "texture2.ppm"};
    for (int i = 0; i < 2; ++i) {
        int w = 0;
        int h = 0;
        std::vector<unsigned char> data;
        if (!loadPPM(files[i], w, h, data)) {
            w = 64;
            h = 64;
            makeChecker(w, h, data);
        }
        g_tex[i] = createTexture(w, h, data);
    }
}

static void setupLights(const float lightPos[4]) {
    glEnable(GL_LIGHTING);
    glEnable(GL_LIGHT0);

    GLfloat ambient[]  = {0.15f, 0.15f, 0.15f, 1.0f};
    GLfloat diffuse[]  = {0.95f, 0.90f, 0.70f, 1.0f};
    GLfloat specular[] = {1.0f, 1.0f, 0.9f, 1.0f};

    glLightfv(GL_LIGHT0, GL_AMBIENT, ambient);
    glLightfv(GL_LIGHT0, GL_DIFFUSE, diffuse);
    glLightfv(GL_LIGHT0, GL_SPECULAR, specular);
    glLightfv(GL_LIGHT0, GL_POSITION, lightPos);

    glEnable(GL_COLOR_MATERIAL);
    glColorMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE);

    GLfloat matSpec[] = {0.6f, 0.6f, 0.6f, 1.0f};
    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, matSpec);
    glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, 32.0f);
}

static void drawAxis(float len) {
    glDisable(GL_LIGHTING);
    glBegin(GL_LINES);
    glColor3f(1, 0, 0); glVertex3f(0, 0, 0); glVertex3f(len, 0, 0);
    glColor3f(0, 1, 0); glVertex3f(0, 0, 0); glVertex3f(0, len, 0);
    glColor3f(0, 0, 1); glVertex3f(0, 0, 0); glVertex3f(0, 0, len);
    glEnd();
    if (g_lightingEnabled) {
        glEnable(GL_LIGHTING);
    }
}

static void drawOrbitCircle(float radius) {
    glDisable(GL_LIGHTING);
    glColor3f(1.0f, 0.9f, 0.4f);
    glBegin(GL_LINE_LOOP);
    for (int i = 0; i < 120; ++i) {
        float a = (2.0f * 3.1415926f * i) / 120.0f;
        glVertex3f(radius * std::cos(a), 0.0f, radius * std::sin(a));
    }
    glEnd();
    if (g_lightingEnabled) {
        glEnable(GL_LIGHTING);
    }
}

static void drawCubeWithGapsColored(float half) {
    const float h = half;
    const float g = g_faceGap;
    const float a = faceAlpha();

    // +X face
    glPushMatrix();
    glTranslatef(h + g, 0, 0);
    glBegin(GL_QUADS);
    glNormal3f(1, 0, 0);
    glColor4f(0.8f, 0.3f, 0.3f, a);
    glVertex3f(0, -h, -h);
    glVertex3f(0,  h, -h);
    glVertex3f(0,  h,  h);
    glVertex3f(0, -h,  h);
    glEnd();
    glPopMatrix();

    // -X face
    glPushMatrix();
    glTranslatef(-h - g, 0, 0);
    glBegin(GL_QUADS);
    glNormal3f(-1, 0, 0);
    glColor4f(0.3f, 0.8f, 0.3f, a);
    glVertex3f(0, -h,  h);
    glVertex3f(0,  h,  h);
    glVertex3f(0,  h, -h);
    glVertex3f(0, -h, -h);
    glEnd();
    glPopMatrix();

    // +Y face
    glPushMatrix();
    glTranslatef(0, h + g, 0);
    glBegin(GL_QUADS);
    glNormal3f(0, 1, 0);
    glColor4f(0.3f, 0.3f, 0.8f, a);
    glVertex3f(-h, 0, -h);
    glVertex3f( h, 0, -h);
    glVertex3f( h, 0,  h);
    glVertex3f(-h, 0,  h);
    glEnd();
    glPopMatrix();

    // -Y face
    glPushMatrix();
    glTranslatef(0, -h - g, 0);
    glBegin(GL_QUADS);
    glNormal3f(0, -1, 0);
    glColor4f(0.8f, 0.8f, 0.3f, a);
    glVertex3f(-h, 0,  h);
    glVertex3f( h, 0,  h);
    glVertex3f( h, 0, -h);
    glVertex3f(-h, 0, -h);
    glEnd();
    glPopMatrix();

    // +Z face
    glPushMatrix();
    glTranslatef(0, 0, h + g);
    glBegin(GL_QUADS);
    glNormal3f(0, 0, 1);
    glColor4f(0.6f, 0.3f, 0.8f, a);
    glVertex3f(-h, -h, 0);
    glVertex3f( h, -h, 0);
    glVertex3f( h,  h, 0);
    glVertex3f(-h,  h, 0);
    glEnd();
    glPopMatrix();

    // -Z face
    glPushMatrix();
    glTranslatef(0, 0, -h - g);
    glBegin(GL_QUADS);
    glNormal3f(0, 0, -1);
    glColor4f(0.3f, 0.7f, 0.8f, a);
    glVertex3f(-h,  h, 0);
    glVertex3f( h,  h, 0);
    glVertex3f( h, -h, 0);
    glVertex3f(-h, -h, 0);
    glEnd();
    glPopMatrix();
}

static void drawCubeWithGapsTextured(float half) {
    const float h = half;
    const float g = g_faceGap;
    const float a = faceAlpha();

    glEnable(GL_TEXTURE_2D);
    glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
    glColor4f(1.0f, 1.0f, 1.0f, a);

    // +X face
    glBindTexture(GL_TEXTURE_2D, g_tex[0]);
    glPushMatrix();
    glTranslatef(h + g, 0, 0);
    glBegin(GL_QUADS);
    glNormal3f(1, 0, 0);
    glTexCoord2f(0, 0); glVertex3f(0, -h, -h);
    glTexCoord2f(1, 0); glVertex3f(0,  h, -h);
    glTexCoord2f(1, 1); glVertex3f(0,  h,  h);
    glTexCoord2f(0, 1); glVertex3f(0, -h,  h);
    glEnd();
    glPopMatrix();

    // -X face
    glBindTexture(GL_TEXTURE_2D, g_tex[1]);
    glPushMatrix();
    glTranslatef(-h - g, 0, 0);
    glBegin(GL_QUADS);
    glNormal3f(-1, 0, 0);
    glTexCoord2f(0, 0); glVertex3f(0, -h,  h);
    glTexCoord2f(1, 0); glVertex3f(0,  h,  h);
    glTexCoord2f(1, 1); glVertex3f(0,  h, -h);
    glTexCoord2f(0, 1); glVertex3f(0, -h, -h);
    glEnd();
    glPopMatrix();

    // +Y face
    glBindTexture(GL_TEXTURE_2D, g_tex[0]);
    glPushMatrix();
    glTranslatef(0, h + g, 0);
    glBegin(GL_QUADS);
    glNormal3f(0, 1, 0);
    glTexCoord2f(0, 0); glVertex3f(-h, 0, -h);
    glTexCoord2f(1, 0); glVertex3f( h, 0, -h);
    glTexCoord2f(1, 1); glVertex3f( h, 0,  h);
    glTexCoord2f(0, 1); glVertex3f(-h, 0,  h);
    glEnd();
    glPopMatrix();

    // -Y face
    glBindTexture(GL_TEXTURE_2D, g_tex[1]);
    glPushMatrix();
    glTranslatef(0, -h - g, 0);
    glBegin(GL_QUADS);
    glNormal3f(0, -1, 0);
    glTexCoord2f(0, 0); glVertex3f(-h, 0,  h);
    glTexCoord2f(1, 0); glVertex3f( h, 0,  h);
    glTexCoord2f(1, 1); glVertex3f( h, 0, -h);
    glTexCoord2f(0, 1); glVertex3f(-h, 0, -h);
    glEnd();
    glPopMatrix();

    // +Z face
    glBindTexture(GL_TEXTURE_2D, g_tex[0]);
    glPushMatrix();
    glTranslatef(0, 0, h + g);
    glBegin(GL_QUADS);
    glNormal3f(0, 0, 1);
    glTexCoord2f(0, 0); glVertex3f(-h, -h, 0);
    glTexCoord2f(1, 0); glVertex3f( h, -h, 0);
    glTexCoord2f(1, 1); glVertex3f( h,  h, 0);
    glTexCoord2f(0, 1); glVertex3f(-h,  h, 0);
    glEnd();
    glPopMatrix();

    // -Z face
    glBindTexture(GL_TEXTURE_2D, g_tex[1]);
    glPushMatrix();
    glTranslatef(0, 0, -h - g);
    glBegin(GL_QUADS);
    glNormal3f(0, 0, -1);
    glTexCoord2f(0, 0); glVertex3f(-h,  h, 0);
    glTexCoord2f(1, 0); glVertex3f( h,  h, 0);
    glTexCoord2f(1, 1); glVertex3f( h, -h, 0);
    glTexCoord2f(0, 1); glVertex3f(-h, -h, 0);
    glEnd();
    glPopMatrix();

    glDisable(GL_TEXTURE_2D);
}

static void drawEdges(float half) {
    glDisable(GL_LIGHTING);
    glColor3f(0.05f, 0.05f, 0.05f);
    glLineWidth(2.0f);

    float h = half + g_faceGap;
    // draw the edges of the bounding box that contains separated faces
    glBegin(GL_LINES);
    // bottom square
    glVertex3f(-h, -h, -h); glVertex3f( h, -h, -h);
    glVertex3f( h, -h, -h); glVertex3f( h, -h,  h);
    glVertex3f( h, -h,  h); glVertex3f(-h, -h,  h);
    glVertex3f(-h, -h,  h); glVertex3f(-h, -h, -h);
    // top square
    glVertex3f(-h,  h, -h); glVertex3f( h,  h, -h);
    glVertex3f( h,  h, -h); glVertex3f( h,  h,  h);
    glVertex3f( h,  h,  h); glVertex3f(-h,  h,  h);
    glVertex3f(-h,  h,  h); glVertex3f(-h,  h, -h);
    // verticals
    glVertex3f(-h, -h, -h); glVertex3f(-h,  h, -h);
    glVertex3f( h, -h, -h); glVertex3f( h,  h, -h);
    glVertex3f( h, -h,  h); glVertex3f( h,  h,  h);
    glVertex3f(-h, -h,  h); glVertex3f(-h,  h,  h);
    glEnd();

    if (g_lightingEnabled) {
        glEnable(GL_LIGHTING);
    }
    glLineWidth(1.0f);
}

static void display() {
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();

    // camera orbit
    glTranslatef(0, 0, -g_camDist);
    glRotatef(g_camPitch, 1, 0, 0);
    glRotatef(g_camYaw, 0, 1, 0);

    drawAxis(1.2f);

    // orbit plane transform
    glPushMatrix();
    glRotatef(g_orbitPlaneYaw, 0, 1, 0);
    glRotatef(g_orbitPlanePitch, 1, 0, 0);

    // orbit radius
    const float orbitR = 2.2f;

    // light position in orbit plane
    float lx = orbitR * std::cos(g_orbitAngle * 3.1415926f / 180.0f);
    float lz = orbitR * std::sin(g_orbitAngle * 3.1415926f / 180.0f);
    float lightPos[] = {lx, 0.0f, lz, 1.0f};

    if (g_lightingEnabled) {
        setupLights(lightPos);
    } else {
        glDisable(GL_LIGHTING);
    }

    // draw orbit circle in the plane
    drawOrbitCircle(orbitR);

    // draw the "sun"
    glPushMatrix();
    glTranslatef(lx, 0.0f, lz);
    glDisable(GL_LIGHTING);
    glColor3f(1.0f, 0.9f, 0.3f);
    glutSolidSphere(0.12, 20, 16);
    if (g_lightingEnabled) {
        glEnable(GL_LIGHTING);
    }
    glPopMatrix();

    // draw cube (with optional transparency)
    if (g_transparent) {
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        glDepthMask(GL_FALSE);
    }

    if (g_useTextures) {
        drawCubeWithGapsTextured(0.8f);
    } else {
        drawCubeWithGapsColored(0.8f);
    }

    if (g_transparent) {
        glDepthMask(GL_TRUE);
        glDisable(GL_BLEND);
    }

    drawEdges(0.8f);

    glPopMatrix();

    glutSwapBuffers();
}

static void reshape(int w, int h) {
    if (h == 0) h = 1;
    glViewport(0, 0, w, h);
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity();
    gluPerspective(60.0, (double)w / (double)h, 0.1, 100.0);
    glMatrixMode(GL_MODELVIEW);
}

static void update() {
    int now = glutGet(GLUT_ELAPSED_TIME);
    float dt = (now - g_lastTimeMs) / 1000.0f;
    g_lastTimeMs = now;

    g_orbitAngle += g_orbitSpeed * dt;
    if (g_orbitAngle > 360.0f) g_orbitAngle -= 360.0f;

    glutPostRedisplay();
}

static void keyboard(unsigned char key, int, int) {
    switch (key) {
        case 27: // ESC
            std::exit(0);
            break;
        case '+':
        case '=':
            g_faceGap += 0.05f;
            if (g_faceGap > 0.8f) g_faceGap = 0.8f;
            break;
        case '-':
        case '_':
            g_faceGap -= 0.05f;
            if (g_faceGap < 0.0f) g_faceGap = 0.0f;
            break;
        case 'q':
            g_orbitSpeed += 5.0f;
            break;
        case 'e':
            g_orbitSpeed -= 5.0f;
            if (g_orbitSpeed < 0.0f) g_orbitSpeed = 0.0f;
            break;
        case 'i':
            g_orbitPlanePitch += 3.0f;
            break;
        case 'k':
            g_orbitPlanePitch -= 3.0f;
            break;
        case 'j':
            g_orbitPlaneYaw += 3.0f;
            break;
        case 'l':
            g_orbitPlaneYaw -= 3.0f;
            break;
        case 'w':
            g_camPitch += 3.0f;
            break;
        case 's':
            g_camPitch -= 3.0f;
            break;
        case 'a':
            g_camYaw += 3.0f;
            break;
        case 'd':
            g_camYaw -= 3.0f;
            break;
        case 'z':
            g_camDist += 0.3f;
            break;
        case 'x':
            g_camDist -= 0.3f;
            if (g_camDist < 2.5f) g_camDist = 2.5f;
            break;
        case 't':
        case 'T':
            g_transparent = !g_transparent;
            break;
        case 'm':
        case 'M':
            g_useTextures = !g_useTextures;
            break;
        case 'o':
        case 'O':
            g_lightingEnabled = !g_lightingEnabled;
            break;
        default:
            break;
    }
    glutPostRedisplay();
}

int main(int argc, char** argv) {
    glutInit(&argc, argv);
    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH);
    glutInitWindowSize(900, 700);
    glutCreateWindow("Lab 5 - Textured Cube");

    glEnable(GL_DEPTH_TEST);
    glEnable(GL_NORMALIZE);
    glShadeModel(GL_FLAT);

    glClearColor(0.95f, 0.95f, 0.98f, 1.0f);

    initTextures();

    g_lastTimeMs = glutGet(GLUT_ELAPSED_TIME);

    glutDisplayFunc(display);
    glutReshapeFunc(reshape);
    glutIdleFunc(update);
    glutKeyboardFunc(keyboard);

    glutMainLoop();
    return 0;
}
